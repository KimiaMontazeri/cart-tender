...
export APP=cart-tender

export BUILD_INFO_PKG="github.com/KimiaMontazeri/cart-tender/pkg/version"
...
run-version:
	go run -ldflags $(LDFLAGS) ./cmd/cart-tender version

run-api:
	go run -ldflags $(LDFLAGS) ./cmd/cart-tender api

build:
	go build -ldflags $(LDFLAGS) ./cmd/cart-tender

install:
	go install -ldflags $(LDFLAGS) ./cmd/cart-tender
